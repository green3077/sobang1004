...
(() => {
  ...
  // 나머지(죽전동 포함, 2023-01-01부로 강서→달서로 재이관)는 전부 달서소방서 관할.
  const DAEGU_DALSEO_GU_GANGSEO_DONGS = ["이곡동", "신당동"];
  // 달성군은 관내현황에 "달성군(다사,하빈,가창 제외)"라고 명시돼 있다 - 다사읍/하빈면은 강서소방서,
  // 가창면은(수성소방서 관내현황에 "수성구 일원, 달성군 가창면 일원"으로 명시) 수성소방서 관할이고
  // 나머지 읍/면(현풍읍/유가읍/논공읍/구지면/옥포읍 등)만 달성소방서 관할.
  const DAEGU_DALSEONG_GUN_GANGSEO_EUPMYEON = ["다사읍", "하빈면"];
  ...
  function guessFireStation(address) {
    ...
    if (/^대구/.test(addr)) {
      ...
      if (gu === "달성군") {
        if (DAEGU_DALSEONG_GUN_GANGSEO_EUPMYEON.some((em) => addr.includes(em))) return "강서소방서";
        if (DAEGU_DALSEONG_GUN_SUSEONG_EUPMYEON.some((em) => addr.includes(em))) return "수성소방서";
      }
