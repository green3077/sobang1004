...
(() => {
  ...
  // 관할119안전센터까지 주소만으로 추정한다. 같은 동 이름이 대구 안에서 다른 구에 또 있는 경우가
  // 있어서(예: "이천동"이 남구 봉덕동 옆에도, 수성구 만촌 쪽에도 있음) 안전센터는 항상 guessFireStation()
  // 으로 먼저 정확한 소방서를 확정한 다음, 그 소방서 소속 목록 안에서만 동 이름을 찾는다 - 이렇게 하면
  // 다른 구에 있는 동명(同名) 동과 절대 섞이지 않는다. 정보가 불확실하거나 너무 잘게 쪼개져 있는 곳
  // (중구 도심 옛 지번식 세분 구역, 북부소방서 산하 세부 구역)은 억지로 추정하지 않고 비워둔다 -
  // 이 칸은 읽기전용이 아니라서 사용자가 직접 채울 수 있다.
  const DAEGU_119_CENTERS = {
    ...
    "서부소방서": [
      ...
      { center: "비산", areas: ["비산1동", "비산2동", "비산3동", "비산4동", "비산6동", "원대1가동", "원대2가동", "원대3가동"] }
    ],
    "수성소방서": [
      { center: "범물", areas: ["범물1동", "범물2동", "삼덕동", "지산1동", "지산2동", "두산동"] },
      { center: "만촌", areas: ["범어4동", "만촌3동", "이천동", "연호동", "고모동"] },
      { center: "황금", areas: ["황금1동", "황금2동", "중동"] },
      { center: "상동", areas: ["상동", "파동"] },
      { center: "수성", areas: ["범어1동", "범어3동", "수성1가", "수성2가", "수성3가", "수성4가"] },
      { center: "무열로", areas: ["만촌1동", "만촌2동", "범어2동"] },
      { center: "고산", areas: ["고산1동", "고산2동", "고산3동", "신매동", "사월동", "욱수동", "노변동", "매호동", "성동", "시지동", "대흥동", "가천동"] },
      { center: "가창", areas: ["가창면"] }
    ],
    "달서소방서": [
      { center: "월성", areas: ["월성1동", "월성2동", "본동"] },
      { center: "본리", areas: ["본리동", "성당동", "두류1동", "두류2동", "두류3동"] },
      { center: "송현", areas: ["상인1동", "상인2동", "상인3동", "송현1동", "송현2동"] },
      { center: "도원", areas: ["도원동", "대곡동", "유천동", "진천동"] },
      { center: "죽전", areas: ["장기동", "용산1동", "용산2동", "죽전동", "감삼동"] }
    ],
    "달성소방서": [
  ...
  function guessFireCenter119(address) {
    ...
    const centers = DAEGU_119_CENTERS[station];
    if (!centers) return "";
    const addr = (address || "").trim();
